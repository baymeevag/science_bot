...
class CorpusCollector:
    ...
    def __init__(self, topic: str):
        self.topic = topic
        self.db_path = os.path.join(os.path.abspath("./"), DB_LOCATION)

        if not os.path.exists(self.db_path):
            os.mkdir(self.db_path)

        self.file_name = os.path.join(
            self.db_path, f"{self.topic}{FILE_POSTFIX}.{FILE_FORMAT}"
        )
    ...
    def dump(self, df: pd.DataFrame):
        df.reset_index(drop=True).to_csv(self.file_name, index=False)
